...
function CadreFitter(){
    /**
     * assume the peg is connected to a node representing the bg (png read or psd group with layers)
     * calculate the rigth transform in order for the bg cadre to fit the current camera. 
     * the cadre object should have the following structure  : 
     *  
     * cadre.frame.x,
     * cadre.frame.y,
     * cadre.frame.width,
     * cadre.frame.height
     * cadre.background.width;
     * cadre.background.height;
     * 
     * @param {string} peg 
     * @param {Object} cadre 
     */
    this.place_peg_according_to_cadre = function(peg_path,cadre){

        MessageLog.trace(JSON.stringify(cadre))

        var input_camera = _buildCameraArg();
        var input_cadre  = _buildCadreArg(cadre);
        var input_bg     = _buildBgArg(cadre);

        const final_coords = _calculate_bg_coords(input_camera, input_bg, input_cadre);
        MessageLog.trace(JSON.stringify(final_coords))

        _applyCoordsToPeg(peg_path,final_coords)
